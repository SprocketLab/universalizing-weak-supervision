...
class NeuralRanker(AbstractNeuralRanker):
    ...
    def ini_ffnns(self, num_features=None, h_dim=30, out_dim=1, num_layers=3, HD_AF='R', HN_AF='R', TL_AF='S',
            ...
            if num_layers > 2:  # middle layers if needed
                if RD:
                    ...
                    for i in range(2, num_layers):
                        ...
                        if BN:  # before applying activation
                            # bn_i = nn.BatchNorm1d(, momentum=1.0, affine=True, track_running_stats=False)
    ...
    def forward(self, batch_ranking):
        if 1 == batch_ranking.size(0): # in order to support batch normalization
            ...
            if self.BN or self.FBN:
                batch_output = batch_ranking
                for layer in self.sf:
                    if layer.__str__().startswith('BatchNorm1d'):
                        # because batchnorm1d dim fixed as dim 1...(TODO: fix it)
