...
def get_data_meta(data_id=None):
    ...
    if data_id in MSLRWEB:
        ...
        has_comment = False
        fold_num = 1
    elif (data_id.upper() in SYNTHETIC):
        max_rele_level = None
        label_type = LABEL_TYPE.Permutation
        num_features = None  ## Bad coding (TODO: fix it) - num_features will be updated in get_default_data_dict
        has_comment = False
...
class LTRDataset(data.Dataset):
    ...
    def __init__(self, split_type, file, data_id=None, data_dict=None, eval_dict=None, batch_size=1, presort=False,
                 shuffle=False, hot=False, buffer=True, num_features=None):
        assert data_id is not None or data_dict is not None
        if data_dict is None: data_dict = self.get_default_data_dict(data_id=data_id, num_features=num_features)
        ...
        if data_dict['data_id'] in MSLETOR or data_dict['data_id'] in MSLRWEB \
            ...
            list_inds = list(range(len(X)))

            for ind in list_inds:
                qid, doc_reprs, doc_labels = qid, X[ind], Y[ind]
                # print('X[ind]', X[ind], 'Y[ind]', Y[ind])
            ...
            print('data_id', data_id, 'is not implemented')
            raise NotImplementedError(f"data_id', {data_id}, 'is not implemented")

    def get_default_data_dict(self, data_id, scaler_id=None, num_features=None):
        ...
        print('get_default_data_dict, data_meta', data_meta)
